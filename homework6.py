...
LOGGER.addHandler(logging.FileHandler('car.log'))
...
class Car(object):
    ...
    GASOLINE_COST_AI92 = 2.2
    GASOLINE_COST_AI95 = 2.4
    DIESEL_COST = 1.8
    ...
    def travel(self, length):
        ...
        while length > 0:
            ...
            if self._tachograph >= self._next_repair_mileage:
                ...
                # if self._recycling = self.DEFAULT_CAR_COST - self._current_cost  :
                # self._current_cost += self._repair_engine_cost
...
class GasolineCar(Car):
    def __init__(self, tank_volume):
        ...
        LOGGER.info("Created gasoline car, tank volume={volume}".format(volume=tank_volume))
        self._type = GASOLINE
        ...
        # self._repair_engine_cost = self.REPAIR_ENGINE_COST

    def refuel(self):
        need_fuel = self._tank - self._fuel_amount
        if self._tachograph <= 50000:
            self._fuel_spent_cost += need_fuel * self.GASOLINE_COST_AI92
        else:
            self._fuel_spent_cost += need_fuel * self.GASOLINE_COST_AI95
        self._fuel_amount += need_fuel
        self._refuel_times += 1
...
class DieselCar(Car):
    def __init__(self, tank_volume):
        ...
        LOGGER.info("Created diesel car, tank volume={volume}".format(volume=tank_volume))
        self._type = DIESEL
